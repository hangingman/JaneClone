...
wxListCtrl(parent, ID_BoardListCtrl, wxDefaultPosition, wxDefaultSize, wxLC_REPORT | wxLC_VIRTUAL) {
     ...
     // クラス自体の目印を設置する
     SetLabel(boardName);
     std::cout << "oldThreadMap size:" << oldThreadMap.size() << std::endl;
     ...
     for (wxString line = datfile.GetFirstLine(); !datfile.Eof(); line = datfile.GetNextLine()) {
	  ...
	  if (hasOldData) {
	       ...
	       std::cout << "hasOldData" << std::endl;
	       std::cout << "check set: listItem.getCheck() = " << listItem.getCheck() << std::endl;
	  }
 ...
 */
     wxString VirtualBoardListCtrl::OnGetItemText(long item, long column) const {
	  ...
	  case COL_CHK:
	       ...
	       //result =  wxString::Format(wxT("%d"), m_vBoardList[item].getCheck());
	       break;
	  case COL_NUM:
 ...
 */
     int VirtualBoardListCtrl::OnGetItemColumnImage(long item, long column) const {
	  ...
	  std::cout << "m_vBoardList[item].getCheck() = " << m_vBoardList[item].getCheck() << std::endl;
