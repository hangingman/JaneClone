...
VERSION=$(awk '{print $2}' ${TRAVIS_BUILD_DIR}/version.txt | paste -s -d"." -)
...
function upload_deb()
{
    ...
    if ! test -e ${TRAVIS_BUILD_DIR}/build/${DEB_FILENAME}; then
...
function upload_dmg()
{
    ...
    cat ${TRAVIS_BUILD_DIR}/build/${DMG_FILENAME} | ${CURL} ${METHOD} ${API}${RESOURCE}
}
