...
wxArrayString MetakitAccessor::GetBoardInfo() {
     ...
     wxMessageBox(wxT("MetakitAccessor::GetBoardInfo"));
     ...
     for (int j = 0; j < vBoardInfo.GetSize(); ++j) {
	  ...
	  //      wxMessageBox(wxString::Format(_("c4_Property p:%s"), p.Name()));
	  //      c4_StringProp pBoardName(p.Name());
	  //      const char* test = pBoardName(vBoardInfo[k]);
	       
	  //      //wxString test = wxString::Format(_("%s"), (const char*) ((c4_StringProp&) p) (r));
	  //      //c4_StringProp strP = ((c4_StringProp&) p) (r);
	  //      wxMessageBox(wxString(test, wxConvUTF8));
	  ...
	  //      wxMessageBox(wxString::Format(_("c4_Property p:%s"), p.Name()));
	  //      wxMessageBox(wxString(p.Name(), wxConvUTF8));
